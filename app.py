...
def main():
    ...
    if menu == "登入":
        st.subheader("登入")
        username = st.text_input("帳號", key=f"login_username_{menu}")
        password = st.text_input("密碼", type="password", key=f"login_password_{menu}")
        
        if st.button("登入", key=f"login_button_{menu}"):
    ...
    # 創建帳號頁面
        ...
        st.subheader("創建帳號")
        new_username = st.text_input("新帳號", key=f"create_account_username_{menu}")
        new_password = st.text_input("新密碼", type="password", key=f"create_account_password_{menu}")

        if st.button("創建帳號", key=f"create_account_button_{menu}"):
            create_account(new_username, new_password)
            st.success("帳號創建成功！")

    # 如果已經登入，顯示記帳選項
    if "logged_in" in st.session_state and st.session_state.logged_in:
        st.sidebar.selectbox("功能", ["新增記帳記錄", "查看記帳記錄", "計算總餘額", "圖表", "登出"], key="accounting_menu")
